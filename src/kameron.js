...
export function buildRecoveryCapsule(input={}){
  ...
  const capsule={
    ...
    swarmer_trust_score:Number(input.swarmer_trust_score??0),
    capability_gate_approval:String(input.capability_gate_approval||'pending'),
    security_policy_version:String(input.security_policy_version||''),
    behavioral_verification_status:String(input.behavioral_verification_status||'pending'),
...
export function recoveryDecision(capsule,policy={}){
  ...
  return {
    ...
    outcome:validation.approved?'resume-approved':'resume-denied',
    validation,
